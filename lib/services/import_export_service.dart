...
import 'package:beatwave/services/storage_backup_service.dart';
...
class ImportExportService {
  ...
  static Future<String?> exportPlaylist(String playlistName,
    ...
    try {
      ...
      final Map<String, dynamic> playlistMap = {
        '_meta': {
          'generated_by': 'BeatWave - Open Source Music Streaming Application',
          'version':
              'v${packageInfo.version}+${int.parse(packageInfo.buildNumber) % 1000}',
          'exportedAt': DateTime.now().toIso8601String(),
          'format': 'v2',
          'note':
              'This file is automatically generated by BeatWave and is intended solely for importing playlists within the application.',
        },
      ...
      final path = await writeToJSON(
          '${playlistDB.name}_BeatWavePlaylist.json', playlistMap,
          path: filePath);
  ...
  static Future<String?> exportTrack(TrackDB trackDB) async {
    try {
      ...
      final path =
          await writeToJSON('${trackDB.title}_BeatWaveSong.json', trackMap);
