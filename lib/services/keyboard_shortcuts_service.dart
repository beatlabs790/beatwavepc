...
import 'package:beatwave/screens/screen/home_views/timer_view.dart';
...
class _KeyboardShortcutsHandlerState extends State<KeyboardShortcutsHandler> {
  ...
  void _startVolumeAdjust(
      LogicalKeyboardKey key, double delta, BeatWaveMusicPlayer player) {
  ...
  void _togglePlayPause(BeatWaveMusicPlayer player) {
  ...
  double _changeVolume(BeatWaveMusicPlayer player, double delta) {
  ...
  (bool, double) _toggleMute(BeatWaveMusicPlayer player) {
    final currentVolume = player.engine.volume;
    if (currentVolume > 0) {
  ...
  LoopMode _cycleLoopMode(BeatWaveMusicPlayer player) {
  ...
  Future<void> _toggleLike(BeatWaveMusicPlayer player) async {
