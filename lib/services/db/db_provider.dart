...
import 'package:beatwave/core/models/exported.dart' as models;
...
import 'package:beatwave/services/db/legacy/legacy_media_id_mapper.dart';
...
class DBProvider {
    ...
    if (Isar.instanceNames.isEmpty) {
      ...
      if (!await dbFile.exists()) {
        await checkAndRestoreDB(dbFile.path, [
          ...
          p.join(appDocDir, 'beatwave_backup_dbv3.isar'),
          p.join(appSuppDir, 'beatwave_backup_dbv3.isar'),
        ]);
      }
  ...
  static Future<String> getDbBackupFilePath() async {
    ...
    backupPath =
        p.join(backupPath, 'beatwaveBackup', 'beatwave_backup_dbv3.json');
    return backupPath;
  }
    ...
    try {
      ...
      final backupPayload = <String, dynamic>{
        '_meta': {
          ...
          'generatedBy': 'BeatWave DBProvider',
